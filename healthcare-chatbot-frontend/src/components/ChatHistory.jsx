...
const ChatHistory = () => {
    ...
    const fetchChatSessions = async () => {
        try {
            ...
            const response = await axios.get('https://mediassist-backend.onrender.com/api/chat-history/sessions', {
    ...
    const fetchSessionDetails = async (sessionId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`https://mediassist-backend.onrender.com/api/chat-history/session/${sessionId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
